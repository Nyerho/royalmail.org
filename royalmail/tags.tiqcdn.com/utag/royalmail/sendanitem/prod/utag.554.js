//tealium universal tag - utag.554 ut4.0.202605011123, Copyright 2026 Tealium.com Inc. All Rights Reserved.
var amzn = window.amzn || [];
var tokenConfig = {};
try {
    (function(id, loader) {
        var u = {
        ...
        u.loaderCallback = function(data_layer) {
            u.initialized = true;
            var ids = u.data.ids.split(' ').join('');
            amzn("setRegion", u.data.region);
            amzn("addTag", ids);
            tokenConfig = {
                gdpr: {
                    enabled: u.forceBoolean(u.data.gdpr.enabled),
                    consent: u.data.gdpr.consent,
                },
                ttl: 9600
            }
            if (u.data.email) {
                tokenConfig.hashedRecords = [{
                    type: 'email',
                    record: u.data.email,
                }];
            }
            if (u.data.email_raw) {
                tokenConfig.email = u.data.email_raw.toLowerCase().trim();
            }
            if (u.data.phonenumber) {
                tokenConfig.phonenumber = u.data.phonenumber.replace(/[^\d]/g, '').trim();
            }
            if (tokenConfig.email || tokenConfig.phonenumber) {
                amzn("setUserData", tokenConfig);
            }
            if (u.data.event_queue.length > 0) {
                u.data.event_queue.forEach(function(eventName) {
                    var eventData = {};
                    if (u.forceBoolean(u.data.generate_event_id)) {
                        var lookup_id = data_layer.tealium_random || Math.random().toFixed(16).substring(2);
                        var eventID = u.data.event_id || u.generateEventID('amazon_event_id', data_layer, lookup_id);
                        eventData.clientDedupeId = eventID
                        utag.globals[lookup_id] = window.utag.globals[lookup_id] || {};
                        window.utag.globals[lookup_id]["amazon_event_id_" + eventName + "_" + u.id] = eventID;
                    }
                    utag.ut.merge(eventData, u.data[eventName], 1);
                    amzn("trackEvent", eventName, eventData);
                });
            }
        };
        u.map = {};
        ...
        u.send = function(utag_event, data_layer) {
            if (u.ev[utag_event] || u.ev.all !== undefined) {
                ...
                b = data_layer;
                u.data = {
                    base_url: 'https://c.amazon-adsystem.com/aat/amzn.js',
                    region: 'EU',
                    ids: '5634b0d5-a28a-41fe-8b0f-dc4ed992f60c',
                ...
                if (!u.data.ids) {
                    utag.DB(u.id + ': Tag not fired: Required attribute Tag IDs not populated');
                    return;
                }
                if (!u.data.region) {
                    utag.DB(u.id + ': Endpoint does not exist, please check your region configuration!');
                }
                if (u.initialized) {
                    u.loaderCallback(data_layer);
                } else {
                    if (u.forceBoolean(u.data.auto_page_tracking)) {
                        u.data.event_queue.push('Pageview');
                    }
                    if (u.forceBoolean(u.data.auto_purchase_tracking) && u.data.order_id) {
                ...
                utag.DB('send:554:COMPLETE');
            }
        };
        utag.o[loader].loader.LOAD(id);
    }('554', 'royalmail.sendanitem'));
} catch (error) {
    utag.DB(error);
}