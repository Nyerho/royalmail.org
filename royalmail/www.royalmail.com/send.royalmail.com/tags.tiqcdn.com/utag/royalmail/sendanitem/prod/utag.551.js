//tealium universal tag - utag.551 ut4.0.202605011123, Copyright 2026 Tealium.com Inc. All Rights Reserved.
! function(w) {
    if (!w.rdt) {
...
try {
    (function(id, loader) {
        ...
        u.queue = [];
        u.generateEventID = function(event_name, data_layer, lookup_id) {
            var hash_input = (data_layer["tealium_timestamp_epoch"] || (function() {
                    var d = new Date();
                    return Math.floor(d.getTime() / 1000);
                })()) + "-" + lookup_id + "-" + event_name + "-" + id,
                event_id = md5.MD5(hash_input).toString();
            return event_id;
        };
        u.getUniqIdForEvent = function(event_name, data_layer, lookup_id) {
            if (!u.toBoolean(u.data.generate_event_id)) return '';
            var event_id = '',
                event_name_prefix = "reddit_pixel_event_id_";
            if (data_layer.tealium_random) {
                event_id = u.data.event_id || u.generateEventID(event_name, data_layer, lookup_id);
                if (window.utag.globals[data_layer.tealium_random]) {
                    window.utag.globals[data_layer.tealium_random][event_name_prefix + event_name] = event_id;
                    window.utag.globals[data_layer.tealium_random][event_name_prefix + event_name + "_" + id] = event_id;
                }
            } else {
                utag.DB("send:551: Event ID Generation is enabled, but the tealium_random value was not found in the data layer. No event ID was generated for " + event_name);
            }
            window.utag.globals[lookup_id][event_name_prefix + event_name] = event_id;
            return event_id;
        };
        ...
        var EVENTS_LIST = ["PageVisit", "ViewContent", "Search", "AddToCart", "AddToWishlist", "Purchase", "Lead", "SignUp"];
        u.EVENTS_LIST_BY_NAME = {
            PageVisit: "PageVisit",
        ...
        u.callback = function(data_layer, lookup_id) {
            u.data.event.forEach(function(eventName) {
                u.sendEvent(eventName, data_layer, lookup_id);
            });
        };
        u.sendEvent = function(eventName, data_layer, lookup_id) {
            var eventData = {};
            switch (eventName) {
                case u.EVENTS_LIST_BY_NAME.AddToCart:
                    {
                        eventData = {
                            itemCount: u.data.itemCount,
                            value: u.data.value,
                            currency: u.data.currency,
                        }
                        break;
                    }
                case u.EVENTS_LIST_BY_NAME.Purchase:
                    {
                        eventData = {
                            itemCount: u.data.itemCount,
                            value: u.data.value,
                            currency: u.data.currency,
                            transactionId: u.data.transactionId,
                        }
                        break;
                    }
                case u.EVENTS_LIST_BY_NAME.AddToWishlist:
                    {
                        eventData = {
                            itemCount: u.data.itemCount,
                            value: u.data.value,
                ...
                case u.EVENTS_LIST_BY_NAME.Lead:
                    {
                        eventData = {
                            value: u.data.value,
                            currency: u.data.currency,
                ...
                case u.EVENTS_LIST_BY_NAME.SignUp:
                    {
                        eventData = {
                            value: u.data.value,
                            currency: u.data.currency,
            ...
            var conversionId = u.getUniqIdForEvent(eventName, data_layer, lookup_id);
            if (conversionId) eventData.conversionId = conversionId;
            utag.ut.merge(eventData, u.data[eventName], 1)
            eventData = u.clearEmptyKeys(eventData);
            if (EVENTS_LIST.indexOf(eventName) > -1) {
                rdt('track', eventName, eventData);
            } else {
                var customData = {
                    customEventName: eventName
                };
                utag.ut.merge(eventData, customData, 1)
        ...
        u.send = function(utag_event, data_layer) {
            if (u.ev[utag_event] || u.ev.all !== undefined) {
                utag.DB("send:551");
                utag.DB(data_layer);
                var a, b, c, d, e, f, h, _event, lookup_id;
                a = utag_event;
                b = data_layer;
                u.data = {
                    account_id: "t2_ax7vaycz2",
                    send_page_visit: "true",
                    generate_event_id: "false",
                    base_url: "//www.redditstatic.com/ads/pixel.js",
                    itemCount: '',
                    value: '',
                    currency: '',
                    email: '',
                ...
                if (u.toBoolean(u.data.generate_event_id)) {
                    lookup_id = data_layer.tealium_random;
                    if (typeof lookup_id !== "undefined") {
                        ...
                        utag.DB("send:551: The tealium_random variable was not defined. Any Reddit Pixel event IDs generated will not be collected by Tealium Collect. Please ensure you are using the latest version of utag.js to generate a tealium_random value.");
                    }
                }
                utag.DB("send:551:EXTENSIONS");
                utag.DB(data_layer);
                ...
                utag.DB("send:551:MAPPINGS");
                utag.DB(u.data);
                u.data.order_id = u.data.order_id || data_layer._corder || "";
                u.data.currency = u.data.currency || data_layer._ccurrency || "";
                if (u.data.event.length === 0 && data_layer._cevent !== undefined) {
                    u.data.event = (u.typeOf(data_layer._cevent) === "array") ? data_layer._cevent.slice(0) : [data_layer._cevent];
                }
                ...
                if (!u.initialized) {
                    var initParams = u.clearEmptyKeys({
                        email: u.data.email,
                    ...
                    if (u.toBoolean(u.data.send_page_visit)) {
                        var conversionId = u.getUniqIdForEvent("PageVisit", data_layer, lookup_id);
                        rdt("track", "PageVisit", conversionId ? {
                            conversionId: conversionId
                        } : undefined);
                    }
                }
                if (u.data.order_id) {
                    var eventData = u.clearEmptyKeys({
                        itemCount: u.data.itemCount,
                        value: u.data.value,
                        currency: u.data.currency,
                        transactionId: u.data.transactionId
                    });
                    var conversionId = u.getUniqIdForEvent("Purchase", data_layer, lookup_id);
                    if (conversionId) eventData.conversionId = conversionId;
                    rdt("track", "Purchase", eventData);
                }
                if (u.initialized) {
                    u.callback(data_layer, lookup_id);
                } else {
                    if (!u.scriptrequested) {
                        ...
                        u.loader({
                            ...
                            src: u.data.base_url,
                            cb: function() {
                                utag.DB('send:551:CALLBACK');
                                u.initialized = true;
                                u.callback(data_layer, lookup_id);
                                utag.DB('send:551:CALLBACK:COMPLETE');
                            },
                            loc: "script",
                            id: "utag_551",
                            attrs: {}
                ...
                utag.DB("send:551:COMPLETE");
            }
        };
        utag.o[loader].loader.LOAD(id);
    }("551", "royalmail.sendanitem"));
} catch (error) {
    utag.DB(error);
}