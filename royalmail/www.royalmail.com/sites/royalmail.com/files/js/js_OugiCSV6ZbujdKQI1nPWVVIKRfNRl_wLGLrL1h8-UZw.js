...
(function($, Drupal, drupalSettings) {
    ...
    Drupal.behaviors.rml_trustarc = {
        attach: function() {
            if (drupalSettings.rml_trustarc.cmp_script_enabled) {
                // Create a container to enable the link for change consent
                // preferences.
                const $target = $('.rml-footer-sitelinks .row');
                if (!$('#teconsent').length && $target.length) {
                    const $teconsentDiv = $('<div/>', {
                        id: 'teconsent',
                        class: 'sitelinkscol'
                    });
                    $target.children().first().after($teconsentDiv);
                }

                setTimeout(function() {
                    $('a.truste_accessible_link').addClass('rml-sitelinks');
                }, 2000);
                ...
                window.addEventListener("message", (event) => {
                    ...
                    if (eventDataJson && eventDataJson.source === "preference_manager") {
                        ...
                        if (eventDataJson.message === "submit_preferences") {
                            Drupal.rml_trustarc.reloadPage();
                        }
                    }
                    // phpcs:ignore
                }, false);
    ...
    Drupal.rml_trustarc.reloadPage = function() {
        setTimeout(() => {
